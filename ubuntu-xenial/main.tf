...
resource "aws_instance" "etcd" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo apt-get install -y --allow-unauthenticated python2.7 etcd",
      "sudo ln -s /usr/bin/python2.7 /usr/bin/python"
    ]
  }
}
...
resource "aws_instance" "master" {
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo chmod +x /tmp/configure-repo.sh",
      "sudo /tmp/configure-repo.sh ${aws_instance.mirror_node.private_ip}",
      "sudo apt-get install -y --allow-unauthenticated python2.7 docker-engine kubelet kubectl kismatic-offline",
      "sudo ln -s /usr/bin/python2.7 /usr/bin/python"
    ]
  }
}
...
resource "aws_instance" "worker" {
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo chmod +x /tmp/configure-repo.sh",
      "sudo /tmp/configure-repo.sh ${aws_instance.mirror_node.private_ip}",
      "sudo apt-get install -y --allow-unauthenticated python2.7 docker-engine kubelet kubectl",
      "sudo ln -s /usr/bin/python2.7 /usr/bin/python"
    ]
  }
}
